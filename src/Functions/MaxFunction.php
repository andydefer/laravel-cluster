...
/**
 * Finds the maximum numeric value within an array extracted from data.
 *
 * This function extracts an array from the data, filters out non-numeric values,
 * and returns the maximum value found. If no numeric values are found,
 * returns 0.
 *
 * @example
 * $max = new MaxFunction();
 * $data = ['scores' => [80, 90, 100]];
 * $max->execute($data, ['scores']); // 100
 * @example
 * $data = ['prices' => [50, 75, 30]];
 * $max->execute($data, ['prices']); // 75
 * @example
 * $data = ['empty' => []];
 * $max->execute($data, ['empty']); // 0
 */
final class MaxFunction extends AbstractAggregateFunction
{
    /**
     * Executes the maximum value calculation on the provided data.
     *
     ...
     * @return float|int The maximum numeric value found, or 0 if none found
     */
