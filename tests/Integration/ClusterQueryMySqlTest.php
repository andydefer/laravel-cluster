...
final class ClusterQueryMySqlTest extends MySqlTestCase
{
    use RefreshDatabase;
    ...
    // ============================================================
    // APPLY TO ELOQUENT TESTS - MySQL (FONCTIONNELS)
    // ============================================================
    ...
    {
        $query = TestCluster::query();
        $this->clusterQuery->applyToEloquent($query, 'clusters', 'status=active & role=admin', DatabaseDriver::MYSQL);
        $results = $query->get();
    ...
    {
        $query = TestCluster::query();
        $this->clusterQuery->applyToEloquent($query, 'clusters', 'status=active | status=pending', DatabaseDriver::MYSQL);
        $results = $query->get();
    ...
    {
        $query = TestCluster::query();
        $this->clusterQuery->applyToEloquent($query, 'clusters', '(status=active | status=pending) & role=admin', DatabaseDriver::MYSQL);
        $results = $query->get();
        $this->assertCount(2, $results);
    }
    ...
    {
        $query = TestCluster::query();
        $this->clusterQuery->applyToEloquent($query, 'clusters', 'lang_fr=yes', DatabaseDriver::MYSQL);
        $results = $query->get();
        $this->assertCount(3, $results);
    }
    ...
    {
        $query = TestCluster::query();
        $this->clusterQuery->applyToEloquent($query, 'clusters', 'lang_fr=no', DatabaseDriver::MYSQL);
        $results = $query->get();
        $this->assertCount(2, $results);
    }
    ...
    {
        $query = TestCluster::query();
        $this->clusterQuery->applyToEloquent($query, 'clusters', 'lang_fr', DatabaseDriver::MYSQL);
        $results = $query->get();
        $this->assertCount(3, $results);
    }
    ...
    {
        $query = TestCluster::query();
        $this->clusterQuery->applyToEloquent($query, 'clusters', 'age>30', DatabaseDriver::MYSQL);
        $results = $query->get();
        $this->assertCount(2, $results);
    }
    ...
    {
        $query = TestCluster::query();
        $this->clusterQuery->applyToEloquent($query, 'clusters', 'age>=35', DatabaseDriver::MYSQL);
        $results = $query->get();
        $this->assertCount(2, $results);
    }

    public function test_apply_to_eloquent_with_lt(): void
    {
        $query = TestCluster::query();
        $this->clusterQuery->applyToEloquent($query, 'clusters', 'age<30', DatabaseDriver::MYSQL);
        $results = $query->get();
    ...
    public function test_apply_to_eloquent_with_lte(): void
    {
        $query = TestCluster::query();
        $this->clusterQuery->applyToEloquent($query, 'clusters', 'age<=25', DatabaseDriver::MYSQL);
        $results = $query->get();
        $this->assertCount(2, $results);
    }
    ...
    {
        $query = TestCluster::query();
        $this->clusterQuery->applyToEloquent($query, 'clusters', '(status=active | status=pending) & lang_fr & age>=25', DatabaseDriver::MYSQL);
        $results = $query->get();
        $this->assertCount(3, $results);
    }
    ...
    {
        $query = TestCluster::query();
        $this->clusterQuery->applyToEloquent($query, 'clusters', 'status=active & role=guest', DatabaseDriver::MYSQL);
        $results = $query->get();
        $this->assertCount(0, $results);
    }

    // ============================================================
    // APPLY TO ELOQUENT TESTS - MySQL avec sous-conditions (JSON_SEARCH)
    // ============================================================

    public function test_apply_to_eloquent_subcondition_simple(): void
    {
        $query = TestCluster::query();
        $this->clusterQuery->applyToEloquent($query, 'clusters', 'addresses[city=Kinshasa]', DatabaseDriver::MYSQL);
        $results = $query->get();
        $this->assertCount(3, $results);
    }
    ...
    {
        $query = TestCluster::query();
        $this->clusterQuery->applyToEloquent($query, 'clusters', 'addresses[city=~Kin%]', DatabaseDriver::MYSQL);
        $results = $query->get();
        $this->assertCount(3, $results);
    }
    ...
    public function test_apply_to_eloquent_subcondition_with_nested_path(): void
    {
        ...
        $query = TestCluster::query();
        $this->clusterQuery->applyToEloquent($query, 'clusters', 'settings.notifications[email=yes]', DatabaseDriver::MYSQL);
        $results = $query->get();
    ...
    {
        $query = TestCluster::query();
        $this->clusterQuery->applyToEloquent($query, 'clusters', 'addresses[]', DatabaseDriver::MYSQL);
        $results = $query->get();
        $this->assertCount(4, $results);
    }
    ...
    {
        $query = TestCluster::query();
        $this->clusterQuery->applyToEloquent($query, 'clusters', 'status=active & addresses[city=Kinshasa]', DatabaseDriver::MYSQL);
        $results = $query->get();
        $this->assertCount(3, $results);
    }
    ...
    public function test_to_sql_with_presence_mysql(): void
    {
        $sql = $this->clusterQuery->toSql('clusters', 'lang_fr', DatabaseDriver::MYSQL);
        $this->assertStringContainsString("JSON_UNQUOTE(JSON_EXTRACT(clusters, '$.\"lang_fr\"')) = 'yes'", $sql);
    }

    public function test_to_sql_with_absence_mysql(): void
    {
        $sql = $this->clusterQuery->toSql('clusters', '!lang_fr', DatabaseDriver::MYSQL);
        $this->assertStringContainsString("JSON_UNQUOTE(JSON_EXTRACT(clusters, '$.\"lang_fr\"')) = 'no'", $sql);
    }

    public function test_to_sql_with_yes_value_mysql(): void
    {
        $sql = $this->clusterQuery->toSql('clusters', 'lang_fr=yes', DatabaseDriver::MYSQL);
        $this->assertStringContainsString("JSON_UNQUOTE(JSON_EXTRACT(clusters, '$.\"lang_fr\"')) = 'yes'", $sql);
    }

    public function test_to_sql_with_no_value_mysql(): void
    {
        $sql = $this->clusterQuery->toSql('clusters', 'lang_fr=no', DatabaseDriver::MYSQL);
        $this->assertStringContainsString("JSON_UNQUOTE(JSON_EXTRACT(clusters, '$.\"lang_fr\"')) = 'no'", $sql);
    }
    ...
    public function test_apply_to_eloquent_dot_notation_simple(): void
    {
        ...
        $query = TestCluster::query();
        $this->clusterQuery->applyToEloquent($query, 'clusters', 'settings.theme=dark', DatabaseDriver::MYSQL);
        $results = $query->get();
        $this->assertCount(1, $results);
    ...
    public function test_apply_to_eloquent_dot_notation_with_subcondition(): void
    {
        ...
        $query = TestCluster::query();
        $this->clusterQuery->applyToEloquent($query, 'clusters', 'settings.notifications[email=yes] & settings.theme=dark', DatabaseDriver::MYSQL);
        $results = $query->get();
        $this->assertCount(1, $results);
        $this->assertEquals('John', $results->first()->clusters['name']);
    }
